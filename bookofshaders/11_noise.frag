...
void main() {
    ...
    float n = (sin(TAU + u_time/5));
    r += (noise2D(st * (100 * n * n *n  * n)) * 0.1 * cos(a + u_time * r) * 10.);
    // r += fract(noise2D(st * (100 * n * n *n )) * 0.1 * cos(a + u_time) * 10.);
    // r += noise2D(st * (0.5 * n * r));
    // r += r * noise2D(st * (5. * n * r * r));

    color = vec3(smoothstep(0.5 - 0.1, 0.52, r) - smoothstep(0.49, 0.5 + 0.001, r));
